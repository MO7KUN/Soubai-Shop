...
async function fetchAllCategoriesWithProducts() {
  try {
    ...
    const categoriesResponse = await fetch(
      "https://sbaishop.com/api/website/categories"
    );
    ...
    for (const category of categories) {
      ...
      try {
        const productsResponse = await fetch(
          `https://sbaishop.com/api/website/category/${category.id}`
        );
        ...
        const products = data.products || []; // Access products from data object

        // Hide loading and show products
        document.querySelector(
          `.products-loading-${category.id}`
        ).style.display = "none";
        ...
        const swiperWrapper =
          productsContainer.querySelector(".swiper-wrapper");
...
function setupCartButtonsForCategory(categoryId) {
  const containers = document.querySelectorAll(
    `.category-products-${categoryId} .add-to-cart-container`
  );
...
function updateProductCartButton(productId) {
  ...
  document
    .querySelectorAll(`.add-to-cart-container[data-id="${productId}"]`)
    .forEach((container) => {
      container.innerHTML =
        quantity > 0
          ? `
